...
function makeRosterCard(name) {
  ...
  el.innerHTML = `
    ...
    <div class="rc-metrics">
      ...
      <div class="rc-metric"><div class="m-val v-batt">100%</div><div class="m-lab">batt</div></div>
...
function updateRosterData(s) {
  for (const [name, fk] of Object.entries(s.forklifts || {})) {
    ...
    const batt = (fk.battery == null) ? 100 : fk.battery;
    r.batt.textContent = Math.round(batt) + "%";
    r.batt.style.color = batt <= 15 ? RED : (batt <= 40 ? AMBER : NV);
